...
class CheckersBenchmark extends Bench.ForkedTime {
	...
	val board1 = buildBoard(Map(Piece(White, Pawn) -> List(46, 47, 48, 49, 50, 41, 45, 36, 38, 39, 40, 33, 34, 35),
								Piece(Black, Pawn) -> List(1, 2, 3, 4, 6, 7, 8, 9, 10, 11, 13, 14, 15),
								Piece(White, Queen) -> List(28),
								Piece(Black, Queen) -> List(20, 26)))

	val board2 = buildBoard(Map(Piece(White, Pawn) -> List(46, 47, 48, 49, 50, 41, 45, 36, 38, 39, 40, 33, 34, 35, 28),
	...
	performance of "PERF TEST Checkers" in {
		measure method "Best move from Alpha beta" in {
			using(Gen.unit("board")) in {
				Unit => new CheckersBrain(AlphaBeta.findBestNode, HyperParameters(6)).bestMove(board2, Black)
			}
		}
	}
